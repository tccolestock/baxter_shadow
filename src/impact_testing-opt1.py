...
class BiotacData(object):
    ...
    # cls.np_average = np.average

    def __init__(self):
        self._angle_window = np.array([0]*5)
        self.angle_mvavg = [0]
        ...
        self._pac1_window = np.array([0]*5) #[0]*5
        self.pac1_base = [0]
        self.pac1_mvavg = [0]
        self.pac1_count = [0]
        ...
        self._pdc_window = np.array([0]*5) #[0]*5
        self.pdc_base = [0]
        self.pdc_mvavg = [0]
        self.pdc_count = [0]
    ...
    def new(self, x, name=None):
        if name == "angle":
            angle_put = self._angle_window.put
            for i in xrange(4):
                angle_put(i, self._angle_window[i+1])
                angle_put(4,x)
            ...
            pac1_put = self._pac1_window.put
            for i in xrange(4):
                pac1_put(i, self._pac1_window[i+1])
                pac1_put(4,x)
            # self._pac1_window.pop()
            # self._pac1_window.insert(0, x)
        elif name == "pdc":
            pdc_put = self._pdc_window.put
            for i in xrange(4):
                pdc_put(i, self._pdc_window[i+1])
                pdc_put(4,x)
    ...
    def _update_mvavg(self, name):
        if name == "angle":
            self.angle_mvavg[0] = np.average(self._angle_window)
            # self.angle_mvavg[0] = sum(self._angle_window)/len(self._angle_window)
        if name == "pac0":
            self.pac0_mvavg[0] = sum(self._pac0_window)/len(self._pac0_window)
        if name == "pac1":
            self.pac1_mvavg[0] = np.average(self._pac1_window)
            # self.pac1_mvavg[0] = sum(self._pac1_window)/len(self._pac1_window)
        if name == "pdc":
            self.pdc_mvavg[0] = np.average(self._pdc_window)
            # self.pdc_mvavg[0] = sum(self._pdc_window)/len(self._pdc_window)
        if name == "tac":
            self.tac_mvavg[0] = sum(self._tac_window)/len(self._tac_window)
        if name == "tdc":
            self.tdc_mvavg[0] = sum(self._tdc_window)/len(self._tdc_window)
...
def listen():
    ...
    # rospy.Subscriber("/rh/tactile_drop/", BiotacAll, callback, queue_size=10, buff_size=100000)
    rate_handle.sleep()
    rospy.spin()
...
def callback(data):
    ...
    # print(ff_elect_count)
    if ff_elect_count[0] < 50:
        ...
        features = np_matrix(ff_electrodes[:24])
        # features = np.matrix(features)  # convert list to numpy matrix
        # % diff from baseline:
        features = ((features.T - ff_elect_base)/ff_elect_base)*100
        angle = fit_e24(features)  # must be a column matrix
        ff_pac1_zero = np_abs(ff_pac1 - ff_pac1_base[0]) #pac1_zero(ff_pac1)
        ff_pdc_zero = np_abs(ff_pdc - ff_pdc_base[0]) #pdc_zero(ff_pdc)
